...
class ReservedRecordUpdater:
    ...
    def update_reserved_record(self, reserved_record: SummaryScoreRecord) -> None:
        """TestCasesテーブルの内容でScoreHistoryテーブルに一時的に挿入されている'reserved_record'を更新します。"""
        detail_records = self.record_read_service.fetch_detail_records_by_id(reserved_record.id)
